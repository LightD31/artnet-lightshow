// Cached two-layer canvas renderer for the auto-show timeline.
// The chart is static for a track; only the playhead moves every frame.
// Keeping those layers separate avoids rewalking thousands of points.
function drawStatic(ctx, data, W, H, dpr = 1) {
...
export function createTimelineRenderer(canvas, background = null) {
  const staticCanvas = background || document.createElement('canvas');
  const ctx = canvas.getContext('2d');
  const staticCtx = staticCanvas.getContext('2d');
  let previousData = null;
  let previousWidth = 0, previousHeight = 0, previousDpr = 0;
  let previousPlayheadPixel = null;
  return function render(data, posMs, options = {}) {
    const width = options.width || canvas.clientWidth || canvas.width || 800;
    const height = options.height || canvas.clientHeight || canvas.height || 200;
    const dpr = options.dpr || window.devicePixelRatio || 1;
    // Evaluate both sides: a short-circuiting `||` would leave the background
    // at its old backing resolution whenever the foreground resized first.
    const canvasResized = sizeCanvas(canvas, width, height, dpr);
    const staticResized = sizeCanvas(staticCanvas, width, height, dpr);
    const resized = canvasResized || staticResized;
    const staticChanged = resized || data !== previousData || width !== previousWidth || height !== previousHeight || dpr !== previousDpr;
    const durMs = data && Number.isFinite(data.duration) ? data.duration * 1000 : 0;
    const playheadPixel = data && durMs > 0 && Number.isFinite(posMs) ? Math.round((Math.max(0, Math.min(durMs, posMs)) / durMs) * width * dpr) : null;
    if (!staticChanged && playheadPixel === previousPlayheadPixel) return false;
    if (staticChanged) {
      staticCtx.setTransform(dpr, 0, 0, dpr, 0, 0);
      drawStatic(staticCtx, data, width, height, dpr);
      previousData = data; previousWidth = width; previousHeight = height; previousDpr = dpr;
    }
    ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    ctx.clearRect(0, 0, width, height);
    ctx.drawImage(staticCanvas, 0, 0, width, height);
    if (data && durMs > 0 && Number.isFinite(posMs) && posMs >= 0 && posMs <= durMs) {
      const px = (posMs / durMs) * width;
      ctx.strokeStyle = '#ffffff'; ctx.lineWidth = 1.5;
      ctx.shadowColor = 'rgba(255,255,255,.7)'; ctx.shadowBlur = 6;
      ctx.beginPath(); ctx.moveTo(px, 0); ctx.lineTo(px, height); ctx.stroke();
      ctx.shadowBlur = 0;
    }
    previousPlayheadPixel = playheadPixel;
    return true;
  };
}
